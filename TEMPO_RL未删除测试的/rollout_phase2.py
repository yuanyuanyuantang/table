...
class MockPolicy(PolicyWrapper):
    """Returns preset responses in order. For smoke testing without a real LLM."""
    ...
    def call(self, messages, tools, temperature=0.7, top_p=0.9):
        if self._idx < len(self._responses):
            content = self._responses[self._idx]
            self._idx += 1
        else:
            content = "<answer>No more mock responses.</answer>"
        self.call_count += 1
...
class FakeToolExecutor(ToolExecutor):
    """Returns canned observations for testing without real tool infrastructure."""

    def __init__(self, table_root: str = "", responses: Optional[Dict[str, str]] = None):
        super().__init__(table_root=table_root)
        self._responses = responses or {}
    ...
    def execute(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "tool_name": tool_name,
            "success": True,
            "content": f"[SUCCESS] Mock result for {tool_name}",
        }
...
def main():
    ...
    parser.add_argument(
        ...
        help="Use MockPolicy for smoke testing"
    )
    parser.add_argument(
    ...
    if args.mock:
        mock_responses = [
            ...
            '<answer>The data shows production increased by 16.96%.</answer>\n'
            '<memory>{"goal": "Find production growth", "tables": ["test.xlsx"], '
            '"key_facts": [{"entity": "Company A", "time": "2010年1月", '
            '"metric": "产量同比增长率", "value": "16.96%", "unit": "%"}]}</memory>',
        ]
        policy = MockPolicy(mock_responses * 10)
        print("[Phase2] Using MockPolicy for smoke testing")
    else:
