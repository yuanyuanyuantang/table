...
1. **忠实性（Faithfulness）**：memory_after 中的每一条事实必须来自以下来源之一：
   ...
   **Derived 字段的 grounding 检查**：derived 中的推导结果必须能从表格数据直接计算或比较得出。
   - ✅ 允许：增长率、差值、排名、占比、大小比较（如"A类价格103.5高于B类99.5"）
   - ❌ 禁止：推测性因果分析，包括但不限于：
     * "可能与...有关" / "可能受...影响"（外部因素推测）
     * "表明...导致..." / "说明...引起..."（因果归因）
     * "预示着..." / "反映出...趋势"（预测性推测）
     * 任何需要外部知识（经济学理论、政策分析等）才能得出的结论
   - 判断标准：该结论是否仅从表格中的数值就能得出？如果答案是否定的，标记为忠实性问题。
...
# Conservative regex patterns for unambiguous speculative causal language in derived.
# These catch the most egregious cases; the LLM verifier handles subtler ones.
_SPECULATIVE_DERIVED_PATTERNS = [
    re.compile(r'可能与.*(?:有关|影响|导致|引起|所致)'),
    re.compile(r'可能(?:预示着|意味着|反映出|暗示着|代表着)'),
    re.compile(r'(?:表明|说明|这表明|这说明).*(?:导致|引起|造成)'),
]


def _check_derived_grounding(derived: list) -> list:
    """Check derived entries for unambiguous speculative causal language.
    Returns a list of issue strings (empty if clean)."""
    issues = []
    for i, entry in enumerate(derived):
        if not isinstance(entry, str):
            continue
        for pat in _SPECULATIVE_DERIVED_PATTERNS:
            if pat.search(entry):
                issues.append(
                    f'derived[{i}] contains speculative causal language: '
                    f'"{entry[:120]}" matched pattern "{pat.pattern}"'
                )
                break
    return issues
...
def main():
    ...
    for i, rec in enumerate(records):
        ...
        # Gate 2.5: Derived entries must not contain speculative causal language.
        # This is a deterministic backstop; the LLM verifier (faithfulness) is the
        # primary judge for subtler cases.
        derived_issues = _check_derived_grounding(memory_after.get('derived', []))
        if derived_issues:
            det_pass = False
            det_issues.extend(derived_issues)
        ...
        if det_pass and effective_overall:
            ...
            if suggestion and 'Verifier failed' not in suggestion:
                ...
                try:
                    ...
                    derived_issues2 = _check_derived_grounding(memory_after2.get('derived', []))
                    if derived_issues2:
                        det_pass2 = False
                        det_issues2.extend(derived_issues2)
