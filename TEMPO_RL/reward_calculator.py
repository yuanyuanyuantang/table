...
class RewardCalculator:
    ...
    def _check_answer_claim_grounded(
        ...
        # Fallback: if text_fact substring match fails (common when answer is
        # natural language but observations are raw table data), extract
        # individual numeric values from the claim and verify they appear in
        # the observation / code / verified sources.
        if not v_ok:
            claim_nums = _extract_numbers_from_text(claim_text)
            if claim_nums:
                matched_nums = 0
                matched_details: List[Dict[str, Any]] = []
                for raw_str, num_val in claim_nums:
                    num_found = False
                    # Check against observation text
                    obs_nums = _extract_numbers_from_text(obs_text)
                    for oraw, oval in obs_nums:
                        if abs(oval - num_val) < 1e-6:
                            num_found = True
                            matched_details.append({
                                "claim_num": raw_str, "value": num_val,
                                "matched_in": "observation", "matched_as": oraw,
                            })
                            break
                    # Check against code outputs
                    if not num_found:
                        for cs in code_outputs:
                            code_nums = _extract_numbers_from_text(cs)
                            for craw, cval in code_nums:
                                if abs(cval - num_val) < 1e-6:
                                    num_found = True
                                    matched_details.append({
                                        "claim_num": raw_str, "value": num_val,
                                        "matched_in": "code_output", "matched_as": craw,
                                    })
                                    break
                            if num_found:
                                break
                    ...
                    if not num_found:
                        for ei in ledger.verified_items:
                            ev = _normalize_number(ei.value or "")
                            if ev is not None and abs(ev - num_val) < 1e-6:
                                num_found = True
                                matched_details.append({
                                    "claim_num": raw_str, "value": num_val,
                                    "matched_in": "verified_evidence",
                                    "matched_as": ei.value,
                                })
                                break
                    if num_found:
                ...
                # If at least 2 numbers match, consider grounded.
                # Ratio threshold is intentionally omitted: answers often contain
                # many computed/derived values that don't appear verbatim in
                # raw table observations — 2+ matching values from different
                # claims is sufficient evidence the model used the real data.
                match_ratio = matched_nums / max(len(claim_nums), 1)
                if matched_nums >= 2:
                    v_ok = True
                    v_audit = {
                        "method": "numeric_fallback",
                        "matched_in": "observation/code_output (numeric)",
                        "claim_numbers_total": len(claim_nums),
                        "claim_numbers_matched": matched_nums,
                    ...
                    v_audit = {
                        ...
                        "numeric_fallback": {
                            ...
                            "claim_numbers_matched": matched_nums,
                            "match_ratio": match_ratio,
                            "reason": f"only {matched_nums}/{len(claim_nums)} numbers matched, need ≥50% and ≥2" if claim_nums else "no numeric values in claim",
                        },
                    }
            else:
                v_audit = {
                    "method": "value_match",
                    "matched_in": None,
                    "numeric_fallback": {"attempted": True, "claim_numbers_total": 0, "reason": "no numeric values in claim"},
                }
