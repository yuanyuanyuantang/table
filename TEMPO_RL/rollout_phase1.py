            ...
            if isinstance(val, str) and val.strip():
                if not self._is_within_root(val, self._table_root):
                    fixed = os.path.normpath(
                        os.path.join(self._table_root, val.lstrip(os.sep))
                    )
    ...
    def run(
        ...
        for sample in samples:
            ...
            for cq in checkout_list:
                ...
                # Use file_path directly (already a clean relative path from
                # the project root, e.g. "dataset/tables/chinese_table/...")
                # Resolve ".." to avoid confusing the model with paths like
                # "dataset/table/../tables/..." which don't exist.
                table_path = os.path.normpath(sample.get("file_path", "")) if self.tool_executor._table_root else ""
        ...
        try:
            for step_idx in range(self.max_tool_steps):
                ...
                if len(tool_calls_in_turn) == 0:
                    # No tool call — try one last time to extract an answer
                    # (handles model output like raw JSON {"answer": ...}
                    #  or code output that contains answer values)
                    last_answer = extract_answer_from_response(content)
                    if last_answer is not None and assistant_answer is None:
                        assistant_answer = {"content": last_answer}
                    # Always save the response for debugging
                    messages.append({"role": "assistant", "content": content})
                    status = "error"
                    error_msg = f"No tool call or answer found in response. Last model output (first 500 chars): {content[:500]}"
                    break
                ...
                # Handle model mistakenly calling "answer" as a tool:
                # some models forget to use <answer> XML tags and instead
                # emit <tool_call>{"tool": "answer", ...}</tool_call>.
                # Treat the arguments as the answer and stop the loop.
                if tool_name and tool_name.lower() == "answer":
                    answer_text = json.dumps(arguments, ensure_ascii=False) if arguments else ""
                    assistant_answer = {"content": answer_text}
                    messages.append({"role": "assistant", "content": content})
                    break
...
def main():
    ...
    try:
        ...
        llm_client = ChatClient(provider=args.provider, config_key=args.config_key)
        llm_enabled = True
    except Exception:
        llm_client = None
        llm_enabled = False
    ...
    if os.path.exists(te_path) and not args.build_te:
        ...
        # Save target evidence so it survives crashes / can be reused
        os.makedirs(os.path.dirname(te_path) or ".", exist_ok=True)
        tes_builder.save(all_tes, te_path)
        print(f"  Saved {len(all_tes)} target evidence sets → {te_path}")
    ...
    if llm_client is not None:
        ...
        print("[Phase 1] ERROR: LLM client unavailable. Cannot create policy.")
        sys.exit(1)
