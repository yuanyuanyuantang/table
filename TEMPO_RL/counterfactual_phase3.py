    ...
    def estimate(
        ...
        for rollout in dialog_rollouts:
            ...
            for sq_rollout in sq_rollouts:
                ...
                n_executed += 1
                print(f"  [cf {n_executed}] sample={sample_id[:50]}... "
                      f"sq{i}→sq{j_star} running paired continuations...", flush=True)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to local HF model/checkpoint (overrides API)"
    )
    parser.add_argument(
        "--device", type=str, default="cuda",
        help="Device for local model (default cuda, or cuda:2, etc.)"
    )
    ...
    # --- Init components ---
    if args.model_path:
        from TEMPO_RL.rollout_phase2 import LocalModelPolicy
        policy = LocalModelPolicy(args.model_path, device=args.device)
        print(f"[Phase3] Using LocalModelPolicy ({args.model_path}, {args.device})")
    else:
