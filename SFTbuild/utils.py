...
def extract_step_plan(reasoning_content: str) -> str:
    ...
    # Normalize temporary paths to <TABLE_ROOT> placeholder
    # /tmp/data/task_xxx/dataset/tables → <TABLE_ROOT>
    text = re.sub(r'/tmp/data/task_\w+/dataset/tables', '<TABLE_ROOT>', text)
    # /tmp/data/task_xxx/ (remaining cases) → <TABLE_ROOT>/
    text = re.sub(r'/tmp/data/task_\w+/', '<TABLE_ROOT>/', text)
        ...
        if isinstance(node, _ast.JoinedStr):
            for v in node.values:
                ...
                if isinstance(v, _ast.FormattedValue):
                    if not _is_static_node(v.value):
                        return False
                else:
                    return False
            return True
    ...
    for node in _ast.walk(tree):
        if isinstance(node, _ast.Call) and isinstance(node.func, _ast.Name) and node.func.id == 'print':
            for arg in node.args:
                ...
                # Handle print('''{"answer": ...}''') — JSON in a string literal
...
def _extract_numeric_values(text: str) -> tuple:
    ...
    # Only matches complete thousand-separated number tokens like "264,500"
    # or "1,234,567.89". Does NOT match CSV data like "101.1,100.6" because
    # the numbers lack the \d{3} comma-group pattern, and does not match
    # list commas in "1,2,3".
    _THOUSAND_SEP_PATTERN = re.compile(
        r'(?<![\d.])[-+]?\d{1,3}(?:,\d{3})+(?:\.\d+)?(?![\d,])'
    )
    def _strip_commas(s: str) -> str:
        return _THOUSAND_SEP_PATTERN.sub(lambda m: m.group(0).replace(',', ''), s)
    ...
    # Match decimal numbers. Preserve sign.
    # No magnitude filter — small values (9.9) and large values (9999999.99)
    # are both legitimate. Year-like noise is handled by the LLM fallback.
    for m in re.finditer(r'(?<!\d)(-?\d+\.\d+)(?!\d)', text):
        val = float(m.group(1))
        values.add(val)

    # Match integers (1+ digits, not after decimal point). Preserve sign.
    # No year filter — year-like values (2019) may be legitimate counts.
    # Year/list-index noise is handled by the LLM fallback.
...
def _verify_evidence(answer_text: str, tagged_observations: list) -> tuple:
    ...
    """
    # Strip statistical base-number declarations so that "上年=100" / "以100为基期"
    # don't generate spurious claims that the value 100 must appear in observations.
    _STRIP_BASE_PATTERNS = [
        r'(上年|基期|基准年|基年)\s*[=＝]\s*\d+',
        r'以\s*\d+\s*(年|年为止)?\s*(为|作为)\s*(基期|基准)',
        r'(上年|同比)\s*[=＝]\s*100',
    ]
    for pat in _STRIP_BASE_PATTERNS:
        answer_text = re.sub(pat, '', answer_text)
    ...
    if not answer_claims and not answer_vals:
    ...
    def _fuzzy_match(value: float, candidates: set) -> bool:
        """Exact match only — deterministic tier only confirms 'obviously the same'.

        Any rounding discrepancy, derivation, or near-miss falls through to the
        LLM fallback, which can assess whether the value is derivable from
        observation data.
        """
    ...
    for claim in answer_claims:
        ...
        if _fuzzy_match(val, obs_values):
            continue
        # For percentage_point units, check if any two obs_values differ by
        # the claim value (e.g. "4 个百分点" may be derived from 103.5 - 99.5).
        if (claim['unit'] == 'percentage_point'
                and abs(val) < 100
                and not _fuzzy_match(val, obs_values)):
            obs_list = sorted(obs_values)
            found = False
            for i in range(len(obs_list)):
                for j in range(i + 1, len(obs_list)):
                    diff = abs(obs_list[j] - obs_list[i])
                    if abs(diff - abs(val)) < 0.02:
                        found = True
                        break
                if found:
                    break
            if found:
                continue  # Derived difference found → not missing
        missing.append(f"{claim['source']} (semantic={val})")

    # ---- Verify raw numeric values ----
    for m in answer_vals:
        if m in obs_values:
            continue
...
_GROUNDING_VERIFIER_PROMPT = """你是一个严格的数据溯源审核专家。检查模型回答中的每一条声明是否能从工具返回结果中找到支撑。
...
## 确定性数值审计（仅供参考，不作为最终判定）
- 数值精确匹配: {det_pass}
- 未在 Observation 中精确出现的数值: {det_missing}

## 任务
逐条检查模型回答中的声明是否能从 Observation 中找到支撑。注意：确定性审计已检查数值是否出现在 Observation 中，你的任务是检查数值的**语义绑定**是否正确。

### 1. 实体-数值绑定
每个数值是否与正确的实体（产品名、类别、指标名、年份）绑定？
- ❌ 示例：Observation 说"乘用车产量 362700，商用车出口量 11690"
         Answer 说"乘用车产量 11690" → 数值正确但绑定到错误实体
- ✅ 示例：Answer 中每个数值的归属（什么产品的什么指标在哪一年）与 Observation 一致

### 2. 方向/趋势正确性
增长/下降、同比/环比的方向是否与 Observation 一致？
- ❌ 示例：Observation 中 A=100, B=200，Answer 却说"A 高于 B"
- ❌ 示例：Observation 显示同比下降，Answer 却说同比增长
- ✅ 示例：大小关系、增减方向与 Observation 中的数据一致

**趋势描述的判断标准**：趋势描述（如"差距在缩小"、"呈下降趋势"、"整体增长"）应从**整体方向**判断，不应因个别年份的波动而否定。只要端点对比和主要走势支持该描述即可。
- ✅ "差距在缩小"：2016年差距2.9 → 2023年差距0，即使2017年短暂扩大到5.6，整体方向正确 → 应通过
- ✅ "始终高于"：8年中有5年高于、3年相等，且答案已注明"到2023年两者相同" → 应通过
- ❌ "始终高于"：如果8年中有5年实际是低于 → 应拦截

### 3. 比较/排名正确性
比较关系、排名顺序是否由 Observation 支持？
- ❌ 示例：Observation 中 X 增长 5%, Y 增长 10%，Answer 却说"X 增长最快"
- ✅ 示例：排名、最大/最小值、趋势判断可以从 Observation 数据中验证
- 注意：如果答案已附带具体数据表，概括性描述只需与数据整体方向一致，个别年份的例外不构成拦截理由

### 4. 无证据声明
Answer 是否包含无法从 Observation 数据中得出的解释、因果或推测？
- ❌ 示例：Observation 只显示库存增加，Answer 额外说"经营压力已经缓解"
- ❌ 示例："可能与整体经济环境有关"、"表明XX导致了YY"、"预示着ZZ"
- ✅ 示例：仅描述数据本身的变化、大小关系、排名等可直接从数据得出的结论

### 重要：区分「数据声明」和「上下文声明」
以下内容不是数据声明，**不需要从 Observation 中验证**，不要标记为 unsupported：
- 用户问题中明确给出的年份、月份、日期（如"2010年1月"来自用户问题，不是数据声明）
- 用户问题中明确给出的统计口径、定义（如"上年=100"、"本月累计"的定义）
- 基于用户问题中已知统计口径的逻辑等价关系（如"1月份本月累计 = 本月数据"，因为1月累计就是1月当月值）
- 列表序号、排名位次（如"排名第3"）

### 重要：概括性/趋势类声明 — 看整体不看细节
当 Answer 中同时包含**具体数据表**和**概括性描述**时，概括性描述只需与数据的**整体方向和主要走势**一致即可：
- "差距在缩小"：端点对比显示缩小即可（中间个别年份波动不构成拦截理由）
- "始终高于"：大部分年份高于即可，且模型通常在上下文中已注明了例外情况
- "增速较快"：整体增长率高于对比对象即可，个别年份增速放缓不影响判断
- "呈下降趋势"：主要走势向下即可，允许中间有小幅反弹

只有当概括性声明的**方向与数据完全相反**或**数值严重错误**时才拦截。措辞不够精确（如用"始终"而实际有3年相等）但整体方向正确 → 通过。

### 注意事项
- 年份、统计基期数字（如"上年=100"）不是数据声明，不需要验证
- 列表序号、排名位次（如"排名第3"）不是数据声明
- Answer 中多余但正确的信息（Observation 有但 Answer 没提到的数据）不算错误
- 判定标准：该声明是否能仅从 Observation 中的表格数据直接得出？

输出 JSON：
{{
  "pass": true/false,
  "binding_issues": ["实体-数值绑定错误的声明"],
  "direction_issues": ["方向/趋势错误的声明"],
  "comparison_issues": ["比较/排名错误的声明"],
  "unsupported_claims": ["无证据的推测/解释/因果声明"],
  "summary": "简要总结审核结果"
}}

任一维度存在问题时 pass 应为 false。"""


def _build_observation_text(tagged_observations: list) -> str:
    """Build compact observation text for LLM prompts."""
    obs_lines = []
    for tool_name, obs in tagged_observations:
        content = obs.get('content', '') if isinstance(obs, dict) else str(obs)
        if content.strip():
...
def verify_evidence_with_fallback(answer_text: str, tagged_observations: list,
                                   client, user_question: str = '',
                                   goal: str = '') -> tuple:
    """Verify answer claims against observations — always uses LLM grounding.

    Two-tier strategy:
      1. Deterministic: extract numeric claims, check exact matches in
         observations.  Result is informational only.
      2. LLM grounding verifier: ALWAYS called (when client is available).
         Checks entity-value binding, direction, comparisons, and
         unsupported causal/interpretive claims.

    Args:
        answer_text: The answer string to verify.
        tagged_observations: List of (tool_name, obs_dict) tuples.
        client: LLM client for grounding verification.
        user_question: The user's sub-question text (provides context so the
            LLM can distinguish data claims from context-derived statements).
        goal: The overall task goal/question (provides broader context).

    Returns:
        (pass: bool, issues: list) — pass is True if LLM confirms all claims
        are grounded, False otherwise. issues lists problems found.
    """
    import json as _json

    # Tier 1: Deterministic numeric audit (informational only)
    det_pass, det_missing = _verify_evidence(answer_text, tagged_observations)

    # Tier 2: LLM grounding verifier — always called
    observations_text = _build_observation_text(tagged_observations)

    prompt = _GROUNDING_VERIFIER_PROMPT.format(
        answer_text=answer_text,
        observations_text=observations_text,
        det_pass='通过' if det_pass else '未通过（以下数值未精确匹配）',
        det_missing=_json.dumps(det_missing, ensure_ascii=False) if det_missing else '(全部精确匹配)',
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            return False, [f'Grounding verifier failed after {max_retries+1} attempts: {last_error}']

    # Cross-validate: pass=true only if no issues exist in any dimension.
    # Reject self-contradictory results like
    # {"pass": true, "comparison_issues": ["A并不高于B"]}.
    issue_keys = [
        'binding_issues',
        'direction_issues',
        'comparison_issues',
    ...
    for key in issue_keys:
        ...
        if not isinstance(items, list):
            ...
            all_issues.extend(str(x) for x in items if x)

    passed = result.get('pass') is True and types_ok and not all_issues

    if passed:
        return True, []

    if not all_issues:
        all_issues.append(result.get('summary', 'Grounding verifier returned inconsistent result'))
...
def revalidate_trajectory(sub_records: list, original_sub_records: list = None,
    ...
    Returns:
        (pass: bool, issues: list, evidence_audit: list)
    """
    import json as _json

    issues = []
    evidence_audit = []  # Per-sq evidence verification audit trail
    ...
    for rec_idx, rec in enumerate(sub_records):
        ...
        if final_answer and sq_tagged_observations:
            ...
            llm_called = use_llm_evidence and evidence_client is not None
            det_pass, det_missing = _verify_evidence(answer_text, sq_tagged_observations)
            evidence_entry = {
                'sq_id': sq_id,
                'llm_called': llm_called,
                'det_pass': det_pass,
                'det_missing': det_missing[:10],
            }
            if llm_called:
                # Always call LLM grounding verifier — deterministic audit is
                # informational only (entity-value binding, direction, comparisons,
                # unsupported claims cannot be checked deterministically).
                ...
                evidence_entry['llm_pass'] = evidence_ok
                evidence_entry['llm_result'] = missing_nums[:10] if not evidence_ok else []
            else:
                evidence_ok = det_pass
                missing_nums = det_missing
                evidence_entry['llm_pass'] = None  # Not called
                evidence_entry['llm_result'] = []
            evidence_audit.append(evidence_entry)
            if not evidence_ok:
                issues.append(
...
def run_cleaning_pipeline(sub_records: list, dialog_idx: int = 0,
    ...
    original = copy.deepcopy(sub_records)  # True deep copy for answer comparison
    ...
    validation_pass, issues, evidence_audit = revalidate_trajectory(
    ...
    report['stage7_evidence_audit'] = evidence_audit
    if verbose:
        if validation_pass:
