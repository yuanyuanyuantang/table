...
## 下一轮子问题（供参考，不要直接回答下一问）
{next_question}

## 任务
基于以上信息，生成本轮求解后的更新记忆。这个记忆将被后续子问题用来理解上下文。

**重要：你的记忆必须为下一轮子问题提供足够的上文信息。** 请确保记忆包含：
- 下一问中可能涉及的实体、类别名称（如"商用车"、"医疗保健类"）
- 对应的数据表格位置（不要记全部原始数值，记表格名称和数据范围即可）
- 时间范围、单位、统计口径等约束条件
- 如果下一问涉及本轮已查询过的表格中的子类别，记录该表格包含哪些子类别

但注意：记忆不是全文缓存，不要把所有原始数值复制进去。下一问所需的详细数据应该由 agent 从表格中读取，记忆只需要让它知道"去哪里找、找什么"。
...
2. **tables（已确认的数据表）**：
   - 保留 memory_before 中所有已确认的表格，不要丢失
   - 如果本轮发现了新的有用表格，追加进来
   - 每条包含表格名称和内容的简要说明
...
核心原则：
- **只增不减**：memory_before 中已有的关键信息不能丢失，除非被本轮明确更正
...
def generate_memory(records: list, client, goal: str = "", verbose: bool = False) -> list:
    ...
    # 按 dialog 分组
    current_dialog = None
    memory_after = dict(INITIAL_MEMORY)
    ...
    for i, rec in enumerate(records):
        ...
        rec['memory_before'] = dict(memory_after)

        # 找下一轮子问题（同 dialog 内），用于指导 forward-looking memory
        next_question = ''
        if i + 1 < len(records):
            next_rec = records[i + 1]
            if (next_rec.get('sample_id') == rec.get('sample_id') and
                next_rec.get('candidate_id') == rec.get('candidate_id')):
                next_question = next_rec.get('user', '')
        ...
        prompt = MEMORY_ANNOTATION_PROMPT.format(
            ...
            next_question=next_question or '（无，这是最后一轮）'
        )
...
def main():
    ...
    parser.add_argument('--subquestions', type=str,
                        ...
                        help='Path to sub-question records (passed_subquestions.jsonl from step4, or repaired_subquestions.jsonl from step5)')
    ...
    if args.dry_run:
        rec = records[0]
        agent_trace = build_agent_trace_text(rec.get('agent_steps', []))
        next_q = records[1].get('user', '') if len(records) > 1 else ''
        prompt = MEMORY_ANNOTATION_PROMPT.format(
